...
import { RoutePointsManager } from './RoutePointsManager'; // Наш конструктор точек

import '../../../styles/admin/ui/Form.css';
import './RouteForm.css'; // Специфичные стили для этой формы
...
export const RouteForm: React.FC<RouteFormProps> = ({
  ...
  const categoryOptions: SelectOption[] = allCategories
    .filter(cat => cat.id !== 1) // Убираем 'Autogenerated'
    .map(cat => ({ value: String(cat.id), label: cat.name }));

  if (isLoading) {
    return <div className="form-status-message">Загрузка данных для формы...</div>;
  }
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="form-container route-form">
      ...
      <div className="route-form-grid">
        ...
        <div className="form-column">
          ...
          <div className="form-group">
            <ImageUpload
              ...
              previewUrl={formData.image_file ? URL.createObjectURL(formData.image_file) : formData.image_preview_url}
        ...
        {/* --- ПРАВАЯ КОЛОНКА: Конструктор точек --- */}
        <div className="form-column">
